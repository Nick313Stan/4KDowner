...
{
    ResolvedDownloadQuality resolved;
    std::string cap = qualityCap;
    if (cap == "Max")
    {
        cap.clear();
    }
    ...
    const int clampedIndex = std::clamp(cardQualityIndex, 0, static_cast<int>(availableQualities.size()) - 1);
    std::string requestedQuality = availableQualities[static_cast<size_t>(clampedIndex)];
    if (!cap.empty())
    {
        requestedQuality = cap;
    }

    if (requestedQuality.empty() || requestedQuality == "Max")
    {
        return resolved;
    }

    const int requestedHeight = ParseQualityHeight(requestedQuality);
    int bestIndex = -1;
    ...
    if (bestIndex >= 0)
    {
        resolved.floor = availableQualities[static_cast<size_t>(bestIndex)];
    }
    else if (requestedHeight <= 0)
    {
        resolved.floor = availableQualities.back();
    }
