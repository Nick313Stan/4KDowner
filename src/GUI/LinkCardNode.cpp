...
namespace
{
std::uint64_t AllocateLinkCardInstanceId()
{
    static std::atomic_uint64_t nextId{1};
    return nextId.fetch_add(1, std::memory_order_relaxed);
}

bool OutputPathExistsOnDisk(const std::string& pathUtf8)
{
    if (pathUtf8.empty())
    {
        return false;
    ...
    return std::filesystem::exists(std::filesystem::u8path(pathUtf8), error);
}
...
LinkCardNode::LinkCardNode(LinkInfo info, bool deferDetailedParse)
    : instanceId_(AllocateLinkCardInstanceId()),
      info_(std::move(info)),
      suppressAutoDetailParse_(deferDetailedParse)
{
    RepairYoutubeThumbnailUrl(info_);
    if (info_.success && info_.formatStreams.empty() && !deferDetailedParse)
    {
...
LinkCardNode::LinkCardNode(LinkCardNode&& other) noexcept
    : instanceId_(other.instanceId_),
      ...
      groupListDismissEnabled_(other.groupListDismissEnabled_),
      dismissedFromList_(other.dismissedFromList_),
      dismissOverlayHovered_(other.dismissOverlayHovered_),
      wasCloseClicked_(other.wasCloseClicked_),
      wasRestoreClicked_(other.wasRestoreClicked_),
      ...
      suppressAutoDetailParse_(other.suppressAutoDetailParse_),
      isDetailParsing_(other.isDetailParsing_),
      detailParseQueued_(other.detailParseQueued_),
      downloadWaitingForDetailParse_(other.downloadWaitingForDetailParse_),
      wasForceParseClicked_(other.wasForceParseClicked_),
      forceParseBounds_(other.forceParseBounds_),
      hasForceParseBounds_(other.hasForceParseBounds_),
      ...
      durationLookupAttempts_(other.durationLookupAttempts_)
{
    ...
    other.detailParseQueued_ = false;
    other.downloadWaitingForDetailParse_ = false;
    other.wasForceParseClicked_ = false;
    other.hasForceParseBounds_ = false;
    other.needsDetailedParse_ = false;
    other.suppressAutoDetailParse_ = false;
    other.durationLookupStarted_ = false;
    other.durationLookupAttempts_ = 0;
    other.dismissedDuringParse_ = false;
    other.groupListDismissEnabled_ = false;
    other.dismissedFromList_ = false;
    other.dismissOverlayHovered_ = false;
    other.wasCloseClicked_ = false;
    other.wasRestoreClicked_ = false;
...
void LinkCardNode::Update(Rectangle bounds, Font font)
{
    ...
    RepairYoutubeThumbnailUrl(info_);
    if (info_.url != urlBefore && info_.success && info_.formatStreams.empty() && !suppressAutoDetailParse_)
    {
        needsDetailedParse_ = true;
    }
    wasForceParseClicked_ = false;
    hasForceParseBounds_ = false;
    ...
    wasCloseClicked_ = false;
    wasRestoreClicked_ = false;

    isHovered_ = UiCursor::IsMouseOverRect(bounds);
    if (dismissedFromList_)
    {
        const Rectangle overlay = CardChrome::DismissOverlayBounds(bounds);
        dismissOverlayHovered_ = CheckCollisionPointRec(GetMousePosition(), overlay);
        if (dismissOverlayHovered_ && IsMouseButtonReleased(MOUSE_BUTTON_LEFT))
        {
            wasRestoreClicked_ = true;
        }
        return;
    }

    const Rectangle closeButton = CardChrome::CloseButtonBounds(bounds);
    if (hasForceParseBounds_ && CheckCollisionPointRec(GetMousePosition(), forceParseBounds_) &&
        IsMouseButtonReleased(MOUSE_BUTTON_LEFT))
    {
        wasForceParseClicked_ = true;
        RequestForceDetailParse();
        return;
    }
    ...
    {
        if (groupListDismissEnabled_ && !parentTabInactive_)
        {
            wasCloseClicked_ = true;
        }
        else
        {
            RequestClose();
    ...
    Rectangle bounds, Font font, bool highlightExcluded, bool highlightCustom, int displayIndex) const
{
    ...
    const bool dismissed = dismissedFromList_;
    const bool inactive = dismissed || parentTabInactive_;
    const Color background = inactive      ? Color{28, 32, 28, 255}
                             : isSelected_ ? Color{17, 30, 17, 255}
                             : isHovered_  ? Color{14, 26, 14, 255}
                                           : Color{10, 18, 10, 255};
    Color border = inactive      ? Color{52, 58, 52, 255}
                   : isSelected_ ? Color{118, 170, 118, 255}
                   : isHovered_  ? Color{90, 124, 90, 255}
                                 : Color{64, 84, 64, 255};
    ...
    const Color titleColor = inactive ? Color{120, 128, 120, 255} : Color{240, 244, 240, 255};
    const Color metaColor = inactive ? Color{90, 100, 90, 255} : Color{150, 170, 150, 255};
    ...
    if (hasThumbnailTexture_)
    {
        ...
        DrawTexturePro(thumbnailTexture_,
                       source,
                       thumbnailBounds,
                       {0.0f, 0.0f},
                       0.0f,
                       inactive ? Color{255, 255, 255, 140} : WHITE);
    }
    else if (isParsing_)
    {
        DrawMiniSpinner(
    ...
    if (info_.isLive)
    {
        CardChrome::DrawPreviewLiveBadge(font, thumbnailBounds);
    }
    UiRoundedClip::Pop();
    ...
    if (isParsing_)
    {
        ...
        if (!dismissed && !parentTabInactive_)
        {
        ...
        if (dismissed)
        {
            CardChrome::DrawDismissOverlay(animatedBounds, font, dismissOverlayHovered_);
        }
        return;
    }
    ...
    if (!info_.success)
    {
        DrawTextEx(font,
                   ...
                   18.0f,
                   0.0f,
                   inactive ? Color{180, 130, 125, 255} : Color{232, 160, 150, 255});
        if (!dismissed && !parentTabInactive_)
        {
            CardChrome::DrawCloseButton(animatedBounds, font);
        }
    ...
    const bool thumbnailHovered = revealPathAvailable && UiCursor::IsMouseOverRect(thumbnailBounds);
    ...
    const bool flatOnlyMeta = IsFlatOnly() && !HasDetailedMetadata() && !info_.isLive;

    const bool sourceHovered = hasSourceBounds_ && UiCursor::IsMouseOverRect(sourceBounds_);
    const bool statusHovered =
        ...
        hasDownloadStatusBounds_ && UiCursor::IsMouseOverRect(downloadStatusBounds_);
    ...
    const bool forceParsing = !inactive && flatOnlyMeta && (isParsing_ || isDetailParsing_);
    const bool showParseQueue = !inactive && flatOnlyMeta && detailParseQueued_ && !forceParsing;
    const std::string parseLabel =
        forceParsing ? "parsing" : (showParseQueue ? "in queue" : (flatOnlyMeta ? "parse needed" : ""));
    const float parseLabelWidth =
        flatOnlyMeta ? MeasureTextEx(font, parseLabel.c_str(), 14.0f, 0.0f).x + (forceParsing ? 16.0f : 0.0f) : 0.0f;
    const float flatParseBlockWidth = flatOnlyMeta ? (separatorWidth + parseLabelWidth) : 0.0f;

    const std::string durationText = info_.isLive ? "LIVE" : info_.duration;
    const bool durationMissing = !flatOnlyMeta && !info_.isLive && ParseDurationSeconds(info_.duration) <= 0.0;
    const bool durationFillParsing =
        !inactive && durationMissing && !info_.url.empty() && (IsDurationLookupPending() || NeedsDurationLookup());
    const bool detailParsing = !inactive && !flatOnlyMeta && (isParsing_ || isDetailParsing_);
    ...
    const float durationBlockWidth =
        flatOnlyMeta ? 0.0f : (separatorWidth + 16.0f + durationWidth + parsingStatusWidth);
    ...
    const float metaBlockWidth = flatParseBlockWidth + durationBlockWidth;
    const float metaMaxX = animatedBounds.x + animatedBounds.width - 34.0f;
    const float maxDetailsWidth = std::max(0.0f, metaMaxX - textX - metaBlockWidth - statusWidth);
    ...
    float afterMetaX = textX + detailsWidth;
    hasForceParseBounds_ = false;
    if (flatOnlyMeta)
    {
        const float parseSeparatorX = afterMetaX;
        DrawTextEx(font, " | ", {parseSeparatorX, animatedBounds.y + 49.0f}, 14.0f, 0.0f, metaColor);
        const Vector2 parsePosition = {parseSeparatorX + separatorWidth, animatedBounds.y + 49.0f};
        const bool parseHovered =
            !forceParsing && !showParseQueue &&
            UiCursor::IsMouseOverRect({parsePosition.x, animatedBounds.y + 45.0f, parseLabelWidth, 22.0f});
        const Color parseColor = parseHovered ? Color{210, 255, 210, 255} : metaColor;
        DrawTextEx(font, parseLabel.c_str(), parsePosition, 14.0f, 0.0f, parseColor);
        if (forceParsing)
        {
            DrawMiniSpinner({parsePosition.x + MeasureTextEx(font, parseLabel.c_str(), 14.0f, 0.0f).x + 8.0f,
                             animatedBounds.y + 58.0f});
        }
        afterMetaX = parsePosition.x + parseLabelWidth;
        if (!forceParsing && !showParseQueue)
        {
            hasForceParseBounds_ = true;
            forceParseBounds_ = {parsePosition.x, animatedBounds.y + 45.0f, parseLabelWidth, 22.0f};
            if (parseHovered)
            {
                UiCursor::RequestHand();
            }
    ...
    {
        const float durationSeparatorX = afterMetaX;
        ...
        afterMetaX = durationPosition.x + durationWidth;
        if (showParsingStatus && !durationSlotParsing)
        {
            DrawTextEx(font, " | ", {afterMetaX + 2.0f, animatedBounds.y + 49.0f}, 14.0f, 0.0f, metaColor);
            const Vector2 parsingPosition = {afterMetaX + 2.0f + separatorWidth, animatedBounds.y + 49.0f};
            ...
            afterMetaX = parsingPosition.x + parsingTextWidth + 16.0f;
        }
    }
    ...
    if (flatOnlyMeta && hasForceParseBounds_)
    {
        Tooltip::DrawIfHovered(font, forceParseBounds_, "force parse");
    }
    else if (!flatOnlyMeta)
    {
        Tooltip::DrawIfHovered(font,
                               {textX + detailsWidth, metaY - 2.0f, afterMetaX - (textX + detailsWidth), metaHeight},
    ...
    if (showStatus)
    {
        const float separatorX = afterMetaX + 2.0f;
...
void LinkCardNode::ApplyOriginalTitle(std::string title)
{
    ...
    title = StripYoutubeLiveStreamTitleSuffix(title);
...
void LinkCardNode::SetGroupListDismissEnabled(bool enabled)
{
    groupListDismissEnabled_ = enabled;
}

void LinkCardNode::SetParentTabInactive(bool inactive)
{
    parentTabInactive_ = inactive;
}

bool LinkCardNode::IsParentTabInactive() const
{
    return parentTabInactive_;
}

bool LinkCardNode::IsDismissed() const
{
    return dismissedFromList_;
}

void LinkCardNode::SetDismissed(bool dismissed)
{
    if (dismissed && !dismissedFromList_)
    {
        CancelActiveParse();
    }
    dismissedFromList_ = dismissed;
    if (!dismissed)
    {
        dismissOverlayHovered_ = false;
    }
}

void LinkCardNode::CancelActiveParse()
{
    if (isParsing_ || isDetailParsing_ || loader_.IsLoading())
    {
        loader_.Cancel();
    }
    isParsing_ = false;
    isDetailParsing_ = false;
    ClearDurationLookupStarted();
    durationLookupAttempts_ = 0;
}

bool LinkCardNode::WasCloseClicked() const
{
    return wasCloseClicked_;
}

bool LinkCardNode::WasRestoreClicked() const
{
    return wasRestoreClicked_;
}

void LinkCardNode::RequestLinkReload()
{
    if (isParsing_ || isDetailParsing_ || loader_.IsLoading())
    {
        loader_.Cancel();
    ...
    isParsing_ = false;
    isDetailParsing_ = false;
    info_.formatStreams.clear();
    info_.availableFormats.clear();
    info_.availableVideoFormats.clear();
    info_.availableAudioFormats.clear();
    info_.availableQualities.clear();
    needsDetailedParse_ = true;
...
bool LinkCardNode::HasCompletedDownload() const
{
    ...
    return OutputPathExistsOnDisk(lastDownloadedPath_);
}
...
bool LinkCardNode::IsLive() const
{
    return info_.isLive;
}

std::uint64_t LinkCardNode::InstanceId() const
{
    return instanceId_;
}
...
void LinkCardNode::RequestDetailedParse()
{
    if (dismissedFromList_)
    {
        return;
    ...
    if (!needsDetailedParse_ && !info_.formatStreams.empty())
    {
        return;
    }
    needsDetailedParse_ = false;
...
void LinkCardNode::RequestForceDetailParse()
{
    if (dismissedFromList_ || info_.url.empty())
    {
        return;
    }
    detailParseQueued_ = false;
    needsDetailedParse_ = true;
    RequestDetailedParse();
}

bool LinkCardNode::WasForceParseClicked() const
{
    return wasForceParseClicked_;
}

bool LinkCardNode::IsFlatOnly() const
{
    return suppressAutoDetailParse_ && info_.formatStreams.empty();
}

bool LinkCardNode::HasDetailedMetadata() const
{
    return !info_.formatStreams.empty();
}

bool LinkCardNode::IsDetailParseQueued() const
{
    return detailParseQueued_;
}

void LinkCardNode::SetDetailParseQueued(bool queued)
{
    detailParseQueued_ = queued;
}

bool LinkCardNode::WantsDownloadAfterDetailParse() const
{
    return downloadWaitingForDetailParse_;
}

void LinkCardNode::SetDownloadWaitingForDetailParse(bool waiting)
{
    downloadWaitingForDetailParse_ = waiting;
}
...
{
    if (dismissedFromList_ || suppressAutoDetailParse_)
    {
        return false;
    }
    return !info_.isLive && !durationLookupStarted_ && durationLookupAttempts_ < 3 &&
           ParseDurationSeconds(info_.duration) <= 0.0 && !info_.url.empty();
}
...
void LinkCardNode::ApplyParseResultIfReady()
{
    ...
    if (isDetailParsing_)
    {
        ...
        {
            const std::string parsedTitle = StripYoutubeLiveStreamTitleSuffix(result.title);
            info_.title = parsedTitle;
            info_.normalizedTitle = NormalizeVideoTitle(parsedTitle);
            if (result.isLive)
            {
                info_.isLive = true;
            }
