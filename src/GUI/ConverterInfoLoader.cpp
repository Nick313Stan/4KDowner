...
namespace
{
    ...
    std::filesystem::path FindFfprobePath()
    {
        const std::array<std::filesystem::path, 2> relativePaths = {
            std::filesystem::path("4kdowner.shared") / "packages" / "ffmpeg" / "bin" / "ffprobe.exe",
            std::filesystem::path("packages") / "ffmpeg" / "bin" / "ffprobe.exe"};
        ...
        return FindExecutableInPath("ffprobe.exe");
    }
    ...
    std::filesystem::path GetConverterPreviewCacheDirectory()
    {
            ...
            localAppData[0] != '\0')
        {
            const std::filesystem::path directory =
                std::filesystem::path(localAppData) / "4KDowner" / "converter-previews";
    ...
    std::string NormalizeContainer(std::string value, const std::filesystem::path& filePath)
    {
        ...
        if (value == "mov" || value == "mp4" || value == "m4a" || value == "3gp")
        {
            return "MP4";
        }
    ...
    std::string ExtractMidVideoPreview(
        ...
        double durationSeconds)
    {
        ...
        const std::wstring command =
            QuoteArgument(ffmpegPath.wstring()) +
            L" -hide_banner -loglevel error -ss " + seekWide +
            L" -i " + QuoteArgument(inputPath.wstring()) +
            L" -frames:v 1 -q:v 3 -y " + QuoteArgument(previewPath.wstring());
        RunCommand(command);
#else
        ...
        const std::string command =
            quotePath(PathUtf8(ffmpegPath)) +
            " -hide_banner -loglevel error -ss " + seekBuffer +
            " -i " + quotePath(PathUtf8(inputPath)) +
            " -frames:v 1 -q:v 3 -y " + quotePath(PathUtf8(previewPath)) + " 2>&1";
        RunCommand(command);
#endif
    ...
    std::string FirstNonEmptyLine(const std::string &text)
    {
        ...
        while (std::getline(stream, line))
        {
            ...
            if (!line.empty() &&
                ...
                line.find("The filename") == std::string::npos &&
                line.find("Invalid argument") == std::string::npos)
            {
                return line;
            }
        }
...
void ConverterInfoLoader::Start(std::string filePath)
{
    ...
    future_ = std::async(std::launch::async, [filePath = std::move(filePath)]
...
ConverterFileInfo ConverterInfoLoader::Load(std::string filePath)
{
    ...
    const std::string videoCodec = FirstNonEmptyLine(RunCommand(
        ffprobeBase + L" -v error -select_streams v:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 " + fileArg));
    const std::string audioCodec = FirstNonEmptyLine(RunCommand(
        ffprobeBase + L" -v error -select_streams a:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 " + fileArg));
#else
    const std::string ffprobe = Quote(PathUtf8(ffprobePath));
    const std::string file = Quote(PathUtf8(inputPath));
    const std::string output = RunCommand(
        ffprobe + " -v error -show_entries format=format_name,duration -of default=noprint_wrappers=1 " + file + " 2>&1");
    const std::string videoCodec = FirstNonEmptyLine(RunCommand(
        ffprobe + " -v error -select_streams v:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 " + file + " 2>&1"));
    const std::string audioCodec = FirstNonEmptyLine(RunCommand(
        ffprobe + " -v error -select_streams a:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 " + file + " 2>&1"));
#endif
