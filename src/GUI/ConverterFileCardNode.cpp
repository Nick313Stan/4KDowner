...
constexpr float kPreviewRoundness = 0.16f;
    ...
    Font font, const std::string& label, const char* tooltip, float& cursorX, float metaY, float metaHeight)
{
    ...
    return UiCursor::IsMouseOverRect(bounds);
}
} // namespace
...
void ConverterFileCardNode::Draw(Rectangle bounds, Font font, int displayIndex) const
{
    ...
    UiRoundedClip::Push(iconBounds, kPreviewRoundness, kPreviewSegments);
    if (isLoading_)
    {
    ...
    const bool previewHovered = revealPathAvailable && UiCursor::IsMouseOverRect(previewHitBounds);
    ...
    const bool statusHovered =
        isConverting_ && hasConvertStatusBounds_ && UiCursor::IsMouseOverRect(convertStatusBounds_);
