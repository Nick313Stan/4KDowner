...
bool IsYtDlpBenignPostProcessLine(const std::string& line)
{
    ...
    if (ContainsInsensitive(line, "pass -k to keep"))
    {
        return true;
    }
    return false;
...
std::string ExtractLastMeaningfulYtDlpOutputLine(const std::string& output)
{
    ...
    while (end > 0)
    {
        ...
        if (line.empty() || IsYtDlpDownloadProgressLine(line) || IsYtDlpBenignPostProcessLine(line))
        {
            continue;
        }
        return line;
