...
public:
    ...
    // Temporary cap for /playlists shelf prefetch + UI (0 = all).
...
private:
    ...
    void BeginShelfPrefetch();
    void PumpShelfPrefetch();
    void StartShelfPrefetchAt(int index);
    void RecordShelfPrefetchFailure(int index);
    void FinalizePlaylistShelfFromPrefetch();
    ...
    int shelfPrefetchTotal_ = 0;
    int shelfPrefetchCompleted_ = 0;
    bool shelfPrefetchWaitingResult_ = false;
    double shelfPrefetchLoadStartedAt_ = 0.0;
    std::vector<LinkGroupInfo> shelfPrefetchResults_;
